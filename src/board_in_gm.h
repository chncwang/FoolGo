...
struct Move {
  ...
  Move(PlayerColor color, PointIndex indx)
      : color_(color),
        indx_(indx) {
  }
};
...
class BoardInGm : private Board<BOARD_LEN> {
 public:
  ...
  PlayerColor LastPlayer() const {
    return last_player_;
  }
  PointIndex KoIndex() const {
    return ko_indx_;
  }
 ...
 private:
  ...
  PointIndex b_pcs_c_;
  BrdInGmDlgt<BOARD_LEN> *delegate_;HashKeyType hash_key_;
...
template<BoardLen BOARD_LEN>
typename BoardInGm<BOARD_LEN>::BitSet NokoPlayableIndexes(
    const BoardInGm<BOARD_LEN> &b, PlayerColor color) {
