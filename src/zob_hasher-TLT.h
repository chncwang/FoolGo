...
inline static HashKeyType HashChange(const BrdChange::Change<T> &chng,
                                     const GetHash &get) {
...
HashKeyType ZobHasher<BOARD_LEN>::GetHash(const BoardInGm<BOARD_LEN> &b) const {
  ...
  result ^=
      (ko_indx == BoardInGm<BOARD_LEN>::NONE) ? noko_hash_ : ko_hash_[ko_indx];

  return result;
}

template<BoardLen BOARD_LEN>
HashKeyType ZobHasher<BOARD_LEN>::GetHash(HashKeyType hash,
                                          const BrdChange &chng) const {
...
ZobHasher<BOARD_LEN>::ZobHasher() {
  ...
  for (int i = 0; i < BoardLenSquare<BOARD_LEN>(); ++i) {
    for (int j = 0; j < 3; ++j)
      board_hash_[i][j] = LRand();
  }

  for (int i = 0; i < 2; ++i)
    player_hash_[i] = LRand();
  for (int i = 0; i < BoardLenSquare<BOARD_LEN>(); ++i)
    ko_hash_[i] = LRand();
  noko_hash_ = LRand();
}
