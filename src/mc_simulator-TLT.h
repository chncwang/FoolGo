...
template<BoardLen BOARD_LEN>
PointIndex MCSimulator<BOARD_LEN>::Simulate(
    const BoardInGm<BOARD_LEN> &input_board) const {
  BoardInGm<BOARD_LEN> bingm;
  bingm.Copy(input_board);
  int count = 0;

  while (!IsEnd(bingm)) {
    if (++count > MoveLimit<BOARD_LEN>())
      break;
//        PRINT_LINE(" ");
//        bingm.PRINT_BOARD();
...
//        PRINT_LINE("cur_player = %d", cur_player);
    ...
    if (play_c > 0) {
      ...
      PointIndex cur_indx = GetXst1<BoardLenSquare<BOARD_LEN>()>(noko_plbl,
                                                                 rand);
