...
class EyeSet {
 public:
  ...
  void SetEye(PointIndex indx, bool v) {
    eyes_[indx] = v;
  }
  void SetRealEye(PointIndex indx, bool v) {
    real_eyes_[indx] = v;
  }
  bool IsEye(PointIndex indx) const {
    return eyes_[indx];
  }
  bool IsRealEye(PointIndex indx) const {
    return real_eyes_[indx];
  }
  PointIndex RealCount() const {
