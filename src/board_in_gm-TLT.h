...
void BoardInGm<BOARD_LEN>::PlayMove(const Move &move) {
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    if (IsFakeEye(eye_sets_[adj_move.color_], adj_move.indx_)) {
      playable_indxs_[oc][adj_indx] = this->IsSuiside(Move(oc, adj_indx));
    }
  }

  for (int i = 0; i < 4; ++i) {
    Position oblq_pos = pos.ObliquePos(i);
    if (!ins.IsInBoard(oblq_pos)) {
      continue;
    }
    if (this->GetPoint(oblq_pos) != EMPTY_POINT) {
      continue;
    }
  ...
  for (int i = 0; i < 4; ++i)
    this->UpdtAtePcsAdjChns(ate_points[i], oc);
...
bool BoardInGm<BOARD_LEN>::IsSuiside(const Move &move) const {
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    if (point == EMPTY_POINT) {
      ...
      if (chain_sets_[oc].GetAirCountByPiece(adj_indx) == 1) {
        return false;
      }
    }
  }
...
void BoardInGm<BOARD_LEN>::PlayBasicMove(
    const Move &move, typename BoardInGm<BOARD_LEN>::PointIndxVector *v,
...
template<BoardLen BOARD_LEN>
typename BoardInGm<BOARD_LEN>::PointIndxVector BoardInGm<BOARD_LEN>::RemoveChain(
    const Move &move) {
  ...
  for (PointIndex indx : trp) {
    ...
    for (int i = 0; i < 2; ++i) {
      playable_indxs_[i].set(indx);
    }
  }

  for (PointIndex indx : trp) {
    this->LetAdjChnsSetAir(indx, true);
  }
...
//    PRINT_LINE("status = %d", status);
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    PointIndex oblq_indx = ins.GetIndex(oblq_pos);
    count = this->IsSelfPieceOrEye(Move(color, oblq_indx)) ? count + 1 : count;
  }
...
void BoardInGm<BOARD_LEN>::UpdtAdjPlblIndxsOfChn(PointIndex indx) {
  ...
  if (air_c == 1) {
//        PRINT_LINE("air_c == 1.");
    PointIndex air_indx = GetLowest1<BoardLenSquare<BOARD_LEN>()>(air_set);
...
void BoardInGm<BOARD_LEN>::UpdtAtePcsAdjChns(
    const BoardInGm<BOARD_LEN>::PointIndxVector &v, PlayerColor ate_color) {
  auto &ins = this->GetPosClcltr();
